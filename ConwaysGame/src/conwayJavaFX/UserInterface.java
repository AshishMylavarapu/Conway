...
public class UserInterface {
	...
	**********************************************************************************************/
	...
	public void runSimulation(){
		...
		//continuously calling the next generation and removing previous generation and adding current generation to the window//
		window.getChildren().remove(oddCanvas);
		oddCanvas = new Pane();
		Board obj1 = new Board();
		nextgen= obj1.nextGen(current); 
	...
	private boolean fileContentsAreValid() {
		...
		//initialising the livecells with the size of line number and having the rows and columns in the array //
		// iterating each element in the loop and adding the position of livecell//
		livecells = new int[lineNumber][2];
		for(int i=0;i<lineNumber;i++) {
			String[] livecells1 = lines.get(i).split(" ");
			livecells[i][0]=Integer.parseInt(livecells1[0]);
			livecells[i][1]=Integer.parseInt(livecells1[1]);
		}
